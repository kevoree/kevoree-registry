 ...
 */
public class RegisterMemberNSHandler extends AbstractHandler {

    private static final Logger log = LoggerFactory.getLogger(RegisterMemberNSHandler.class.getSimpleName());

    public RegisterMemberNSHandler(Context context) {
        super(context, true);
    }
    ...
    protected void handleJson(HttpServerExchange exchange) throws Exception {
        ...
        try {
            ...
            String member = data.get("member").asString();
            ...
            if (fqn != null && !fqn.trim().isEmpty()) {
                if (member != null && !member.trim().isEmpty()) {
                    ...
                    if (nsService.isOwner(fqn, user.getId())) {
                        nsService.register(fqn, member);
                        ResponseHelper.ok(exchange);
                    } else {
                        exchange.setResponseCode(StatusCodes.FORBIDDEN);
                        JsonObject response = new JsonObject();
                        response.add("error", "User \""+user.getId()+"\" does not own namespace \""+fqn+"\"");
                        ResponseHelper.json(exchange, response);
                    }
                } else {
                    exchange.setResponseCode(StatusCodes.BAD_REQUEST);
                    JsonObject response = new JsonObject();
                    response.add("error", "Given user id is null or empty");
                    ResponseHelper.json(exchange, response);
                }
            } else {
                exchange.setResponseCode(StatusCodes.BAD_REQUEST);
