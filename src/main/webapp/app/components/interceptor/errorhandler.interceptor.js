...
errorHandlerInterceptor.$inject = ['$q', '$window', 'AlertService'];

function errorHandlerInterceptor($q, $window, AlertService) {
  var service = {
    ...
    if (response.status >= 400 && response.status < 500) {
      if (!(response.status === 401 && (response.data === '' || (response.data.path && response.data.path.indexOf('api/account') === 0)))) {
        switch (response.status) {
          ...
          case 401:
            AlertService.warning('global.messages.account.disconnected');
            break;
